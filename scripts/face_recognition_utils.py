...
class DlibFaceRecognizer:
    ...
    def detect_faces(self, image):
        ...
        """
        # Đảm bảo ảnh là grayscale cho dlib
        ...
        # Phát hiện khuôn mặt
        if self.use_cnn:
            # CNN detector trả về mmod_rectangles
            dets = self.face_detector(gray_image)
            # Chuyển sang định dạng rectangle thông thường
            faces = [d.rect for d in dets]
        else:
            # HOG detector trả về rectangles
...
def process_frames_with_dlib():
    ...
    face_dataset_name = "video_dataset_faces_dlib"
    ...
    for i in range(batch_count):
        ...
        for sample in batch_view:
            try:
                ...
                if face_embeddings:
                    sample["dlib_face_embeddings"] = face_embeddings
                    sample.save()
